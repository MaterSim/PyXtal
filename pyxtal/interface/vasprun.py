...
class vasprun:
    ...
    def parse_vaspxml(self, xml_object):
        ...
        for child in xml_object.iterchildren():
            ...
            if child.tag == "incar":
                ...
                (
                    self.values["pseudo_potential"],
                    self.values["potcar_symbols"],
                    self.values["valence"],
                    self.values["mass"],
    ...
    def export_poscar(self, filename):
        ...
        with open(filename, "w") as f:
            ...
            f.write("1.0\n")
            f.write(
                f"{latt[0][0]:12.6f} {latt[0][1]:12.6f} {latt[0][2]:12.6f}\n"
            )
            f.write(
                f"{latt[1][0]:12.6f} {latt[1][1]:12.6f} {latt[1][2]:12.6f}\n"
            )
            f.write(
                f"{latt[2][0]:12.6f} {latt[2][1]:12.6f} {latt[2][2]:12.6f}\n"
            )
            for key in comp:
                f.write(f"{key:4s}")
            f.write("\n")
            ...
            for coor in pos:
                f.write(
                    f"{coor[0]:12.6f} {coor[1]:12.6f} {coor[2]:12.6f}\n"
                )
    ...
    def plot_dos(self, filename=None, smear=None, styles="t", xlim=None, dpi=300):
        ...
        leg1 = ax.legend(
            lines1, list(labels1), fancybox=True, loc="upper right"
        )
