...
from crystal_first_bug import random_cluster
#from crystal_last_good import random_cluster
...
class LJ_prediction():
    ...
    def relaxation(self, dim, pgs, method, ind):
        ...
        if ground1 or ground2:
            logging.info('ID: {:4d} PG initial: {:4s} relaxed: {:4s} {:4s} Energy: {:9.3f} {:9.3f}++++++++'.\
                   format(ind, pg0, pg1, pg2, energy1, energy2))
        #elif ind%100 == 0:
        print('ID: {:4d} PG initial: {:4s} relaxed: {:4s} {:4s} Energy: {:9.3f} {:9.3f}'.\
                   format(ind, pg0, pg1, pg2, energy1, energy2))
        return res
