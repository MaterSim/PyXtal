...
def get_inertia_tensor(mol):
    #Calculate the inertia tensor for a PMG Molecule object
    positions = []
    masses = []
    for x in mol:
        positions.append(x.coords)
        masses.append(x.specie.number)
    positions -= mol.center_of_mass  # translate center of mass to origin
...
#Symmetrize ch4
ch4 = pga_ch4.symmetrize_molecule()['sym_mol']
pga_ch4 = PointGroupAnalyzer(ch4)
