...
One can specify the geometry of molecule in several ways. 

* Load from PyXtal library
.. code-block:: Python

>>> from pyxtal.database.collection import Collection
>>> test=Collection('molecules')
>>> list(test)
    ['C60', 'H2O', 'CH4', 'NH3', 'benzene', 'naphthalene', 'teracene', .......]

To use these molecules from the library, one just needs to give the name
...
* Load from a file (using mol_from_file, which is in molecule.py)

.. code-block:: Python
	
	from pyxtal.molecular_crystal import molecular_crystal
	my_crystal = molecular_crystal(36, ['H2O'], [4], 1.0) # to change
  
* Load from a string (using mol_from_string, which is also in molecule.py)
